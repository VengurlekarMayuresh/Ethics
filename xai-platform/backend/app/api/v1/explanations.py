...
async def request_local_explanation(
    ...
    try:
        ...
        # Get the latest prediction for this user or create one
        prediction_id = None
        if input_data:
            ...
            if last_pred:
                prediction_id = str(last_pred["_id"])

        if not prediction_id:
            raise HTTPException(status_code=400, detail="No prediction found. Provide input_data or run a prediction first.")

        # Trigger async SHAP computation
        task = celery_app.send_task("compute_shap_values", args=[prediction_id, model_id])
